...
database_dir = os.path.join(current_dir, f'../database/kobis')
...
cursor.execute("DROP TABLE IF EXISTS kobis_boxoffice")
cursor.execute("""
               CREATE TABLE IF NOT EXISTS kobis_boxoffice
               AS 
               SELECT * 
               FROM read_parquet('s3://sms-warehouse/kobis/*/*/*/*.parquet')
               """)
